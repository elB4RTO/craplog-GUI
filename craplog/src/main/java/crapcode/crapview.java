...
public class crapview {
    ...
    public static String viewLogFile(
        ...
        Path path = Paths.get( String.format("%s/%s",logs_dir,file_name) );
        // attempt reading
        try {
            InputStream f_in = Files.newInputStream( path );
            BufferedInputStream buff_in = new BufferedInputStream( f_in );
            // get data
            read = new String( buff_in.readAllBytes() );
            // close
            buff_in.close();
            f_in.close();
        
        } catch (FileNotFoundException e) {
            JOptionPane.showMessageDialog(null, String.format("An error occured while searching for log file:\n'%s'",path), "File not found", 0);

        } catch (IOException e) {
            JOptionPane.showMessageDialog(null, String.format("An error occured while reading log file:\n'%s'",path), "Error reading", 0);
        }
            ...
            for ( File log_file : logs_dir.listFiles(logFileFilter) ) {
                ...
                if ( log_file.exists() ) {
                    ...
                    if ( log_file.isDirectory() ) {
                        ...
                        &&  (file_name.startsWith("access.log")
